...
默认图只展示主链路 `Project -> Task -> Worktree -> Thread Role`。HTML dashboard 把 Project 保持为页面上下文，task/worktree/thread 支持 route spotlight；dependency 和 `th-project-hub` audit/routing 入口不进入 ownership 图；risk、nextStep、validation、handoff 放在详情面板，不画成主图节点。状态和健康度通过文本 badge 与视觉 class 表达。归档任务默认隐藏，并显示 `Archived hidden: N`；只有需要查看归档上下文时才传 `--include-archive`。
