...
直接从 plan 到 execution 仍然是主路径：discussion 或 research thread 应该给用户完整 execution prompt 和计划，用户可以直接贴给 Primary Execution Thread。V3.9 新增的 `load-handoff` 用于后续接续：当旧 thread 太长、不可用，或拆出 review/validation 时，新 thread 可以读取紧凑回执或指定 handoff section，而不是复制完整 handoff Markdown。
...
```text
Use $agent-workflow-hub to 继续 AWH V3.9 handoff loading audit.
```
...
## Thread Continuity 与 Handoff Loading

`load-handoff` 让新 thread 通过 `--task-id`、`--query` 或当前 worktree 读取已保存的 workflow state。它是显式、可审计的接续工具：

- 默认 `--mode compact` 只返回紧凑回执、下一步、风险/阻塞和 continue phrase，不返回完整 Markdown handoff。
- `--mode section --section validation` 或其他 section 只读取指定 `##` section，适合 review、validation 或讨论补充。
- 只有用户明确要求时才用 `--mode full` 返回完整 handoff。
- `--query` 歧义时返回候选和一句消歧问题，不猜。
- 每次读取都会向 `events.jsonl` 写入轻量 telemetry；记录 mode、section、reason、role 和内容长度，不记录 handoff 正文。

示例：

```text
Use $agent-workflow-hub first. If only $context-handoff is available, use it as the compatible entrypoint.
继续 AWH V3.9 handoff loading audit。
```

Validation thread 可以只读验证 section：

```text
load-handoff --query "继续 AWH V3.9 handoff loading audit" --mode section --section validation --reason validation-needed
```

更多说明见 [docs/workflows/thread-continuity.md](./docs/workflows/thread-continuity.md) 和 [docs/reference/handoff-loading.md](./docs/reference/handoff-loading.md)。
...
- `load-handoff`：读取已保存 handoff context。默认返回 compact receipt；section/full 模式必须显式请求，并写入 audit telemetry。
...
`weekly-report` 面向项目进展汇报。`eval-report` 面向 Agent Workflow Hub 工具效果：usage counts、recovery health、routing health、handoff loading behavior、coverage 和 proxy efficiency metrics。它刻意不声明精确 token 节省。
