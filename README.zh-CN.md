...
- `visualize-project`：在 sidecar 的 `reports/` 目录生成 Markdown + Mermaid 项目关系图和配套 JSON。用户要求“可视化项目”或“显示项目图”时使用。
...
skill 会运行 `visualize-project`，在本机 sidecar 的 `reports/` 目录写入 Markdown 和 JSON 报告。默认回复只贴 Mermaid 图、Legend、详情表和 needs-attention 摘要，不贴完整 JSON。

默认图只展示主链路 `Project -> Task -> Worktree -> Thread Role`。状态和健康度通过节点 badge/class 与详情表表达，不作为默认 state 节点。归档任务默认隐藏，并显示 `Archived hidden: N`；只有需要查看归档上下文时才传 `--include-archive`。
