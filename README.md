...
The default graph shows the main relationship chain `Project -> Task -> Worktree -> Thread Role`. The HTML dashboard keeps Project as page context, uses a route spotlight interaction for task/worktree/thread nodes, keeps dependencies and `th-project-hub` audit/routing outside the ownership graph, and puts risk/nextStep/validation/handoff in the detail panel instead of drawing them as graph nodes. Health is represented through text badges and visual classes. Visualization details keep canonical `threadRole` separate from human-facing `threadLabel`. Archived tasks are hidden by default and summarized as `Archived hidden: N`; pass `--include-archive` only when archived context should be shown.
