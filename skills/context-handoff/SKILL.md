...
Codex thread discovery is best-effort audit evidence, not workflow truth. The sidecar `threads[]` registry remains the workflow source of truth; local Codex metadata can only prove that a thread exists, where it ran, when it was recently updated, and possible spawn edges. Do not read or paste full thread transcripts for routine reconciliation.

When the user says the Project Hub map, sidecar, or thread state is stale, run `reconcile-threads` before asking them to manually inspect threads. The script scans local Codex SQLite/session metadata and compares only ids/metadata with `threads[].codexThreadId`; the scan itself is local and low-token because only the JSON result returned to the agent enters context. If `refreshRequests` are returned and Codex `send_message_to_thread` is available in the current host, send each short refresh prompt to the target thread. If the tool is unavailable, report the refresh prompts as manual actions. Never read complete JSONL transcripts by default.
...
- `scan-codex-threads`: Read local Codex thread metadata and report sidecar mismatches without writing sidecar state. It returns mismatch ids and minimal metadata only; title is opt-in, preview/chat content is not returned.
- `reconcile-threads`: Compare local Codex thread metadata with sidecar `threads[].codexThreadId`, cache only mismatch audit state, and return short `refreshRequests` prompts for stale or unregistered threads.
- `refresh-thread-registration`: Run inside the target thread to register or refresh its `codexThreadId`, role, label, purpose, scope, and last-seen metadata in sidecar.
...
- `visualize-project`: Generate latest-only Markdown + Mermaid, companion JSON, and a static HTML Project Hub dashboard under the sidecar `reports/` directory. CLI automation does not open windows by default; in interactive Codex conversations where the user asks to visualize or see the project graph, run `visualize-project --open-dashboard`. Use the HTML dashboard as the default human-facing entrypoint: reply with a clickable Markdown link from `dashboardUrl` when present, otherwise `dashboardPath` or `reportPaths.html`, then the Markdown path when useful. Do not paste the full JSON, full HTML, or full Markdown by default. The HTML dashboard keeps Project as page context, shows `Task -> Thread -> Environment(optional)`, supports route spotlight, uses text health badges, hides archive by default, keeps dependencies plus `th-project-hub` outside the ownership graph, and displays canonical `threadRole` separately from `threadLabel`.
...
- If the user says sidecar/thread state is inaccurate, stale, or missing a Codex thread, run `reconcile-threads`. Send returned `refreshRequests` through `send_message_to_thread` only when that tool is available; otherwise give the manual prompt. Do not call `read_thread` or inspect full JSONL histories unless the user explicitly asks for a specific thread diagnosis.
...
- If the user says `visualize project`, `show project graph`, `可视化项目`, `显示项目图`, `项目关系图`, or `看一下项目全局`, run `visualize-project --open-dashboard` in interactive Codex sessions. Reply with a clickable HTML dashboard Markdown link first (`dashboardUrl` when present, otherwise `dashboardPath` or `reportPaths.html`), then the Markdown report path if useful; avoid pasting the full JSON, full HTML, or full Markdown unless asked.
